...
class Vk
{
    ...
    {
        return $this->api('execute', ['code' => $code]);
    }
    ...
    */
    ...
    public function upload_photo($gid = 0, $files = [], $return_ids = false)
    {
        ...
        $data_json = $this->api('photos.getWallUploadServer', ['group_id' => intval($gid)]);
        ...
        foreach ($temp[0] as $key => $data) {
            ...
            if ($path) {
                $files['file' . ($key + 1)] = (class_exists('CURLFile',
                    false)) ? new CURLFile(realpath($data)) : '@' . realpath($data);
            }
        }
    ...
    public function upload_doc($gid = false, $file)
    {
        ...
        $data_json = $this->api('docs.getUploadServer', ['gid' => intval($gid)]);
